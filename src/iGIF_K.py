...
class iGIF_Ca(GIF_Ca) :

    """
    Abstract class to define the:
    inactivating Generalized Integrate and Fire models (iGIF) with voltage-dependent
    calcium current (Ca) = iGIF_Ca.

    In the iGIF model, the firing threshold V_T(t) is given by:

    V_T(t) = Vt_star + sum_j gamma(t-\hat t_j) + theta(t)

    and \hat t_j denote the spike times and theta(t) is given by:

    tau_theta dtheta/dt = -theta + f(V)

    Classes that inherit from iGIF_Ca must specify the nature of the coupling f(V) (this function can eg be defined as
    a liner sum of rectangular basis functions to perform a nonparametric fit, as in the class iGIF_Ca_NP).
    """
